...
def _dequant_selected_hidden_states(
    ...
    m = token_idx.numel()
    a = hidden_states.index_select(0, token_idx).to(torch.bfloat16).view(
        m, NUM_HIDDEN_BLOCKS, BLOCK_SIZE
    )
    s = (
        hidden_states_scale.to(torch.bfloat16)
        .transpose(0, 1)
        .contiguous()
        .index_select(0, token_idx)
        .unsqueeze(-1)
    )
    return (a * s).view(m, HIDDEN_SIZE)
...
def _dequant_selected_w13_t(
    ...
    w = gemm1_weights.index_select(0, active_local_ids).to(torch.bfloat16).view(
        n,
    ...
    s = gemm1_weights_scale.index_select(0, active_local_ids).to(torch.bfloat16).view(
        n,
        NUM_GEMM1_OUT_BLOCKS,
        NUM_HIDDEN_BLOCKS,
    )
    deq = (w * s[:, :, None, :, None]).view(n, 2 * INTERMEDIATE_SIZE, HIDDEN_SIZE)
    return deq.transpose(1, 2).contiguous()
...
def _dequant_selected_w2_t(
    ...
    n = active_local_ids.numel()
    w = gemm2_weights.index_select(0, active_local_ids).to(torch.bfloat16).view(
        n,
        NUM_HIDDEN_BLOCKS,
        BLOCK_SIZE,
    ...
    s = gemm2_weights_scale.index_select(0, active_local_ids).to(torch.bfloat16).view(
        n,
        NUM_HIDDEN_BLOCKS,
        NUM_INTERMEDIATE_BLOCKS,
    )
...
def _build_local_plan(
    ...
    token_flat = nz[:, 0]
    k_slot = nz[:, 1]
    expert_flat = local_idx[token_flat, k_slot]
    w_flat = topk_weights[token_flat, k_slot]

    order = torch.argsort(expert_flat)
