...
def monitor_process(interval=10):
        ...
        while getattr(_monitor, 'running', True):
            try:
                ...
                try:
                    ...
                    for gpu in stats:
                        gpus.append(f"GPU{gpu.index}: {gpu.utilization}% util, {gpu.memory_used}/{gpu.memory_total}MB")
                except:
                    pass
...
def ncu_profile(solution: Solution, workload) -> str:
    ...
    print("[WARN] This performs detailed hardware-level profiling and may take ~5 minutes...")
    ...
    monitor_thread = monitor_process(interval=15)
    try:
    ...
    print("\n" + "="*70)
    print(f"[COMPLETE] NCU profiling finished in {elapsed:.1f} seconds ({elapsed/60:.1f} minutes)")
    print("="*70 + "\n")
